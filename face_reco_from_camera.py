...
class Face_Recognizer:
    def __init__(self):
        ...
        self.face_num_known_list = []                    # 存储录入人脸编号 / Save the name of faces in database
        self.face_sex_known_list = []                    # 存储录入人脸名字 / Save the name of faces in database
        ...
        if os.path.exists("data/features_all.csv"):
            ...
            for i in range(csv_rd.shape[0]):
                ...
                if os.path.exists("data.txt"):
                    path_features_known_txt = "data.txt"
                    with open(path_features_known_txt, "r") as f:  # 打开文件
                        """strip函数原型
                        ...
                        while line:
                            for i in range(3):
                                if i == 0:
                                    self.face_name_known_list.append(line.rstrip())
                                    line = f.readline()
                                    continue
                                if i == 1:
                                    self.face_num_known_list.append(line.rstrip())
                                    line = f.readline()
                                    continue
                                if i == 2:
                                    self.face_sex_known_list.append(line.rstrip())
                                    line = f.readline()
                                    continue
                        f.close()
    ...
    def show_chinese_name(self):
        ...
        if self.current_frame_face_cnt >= 1:
            for j in range(self.current_frame_face_cnt):
                self.face_name_known_list[j].encode('utf-8').decode()
